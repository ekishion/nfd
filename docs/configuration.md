...
本文档详细说明所有环境变量、私密群聊/话题路由配置、动态控制面板参数、关键词/正则审查规则、快捷回复、离开模式以及数据文件的维护方式。
...
| `ENV_ALERT_THREAD_ID` | 可选 | - | 拦截告警通知投递的目标话题 ID（例如群内的 `🛡️ 拦截通知` 专属话题） |
...
### 场景一：将所有留言转发到一个私密群组
1. 在 Telegram 中新建一个私密群组，并将你的 Bot 拉入群内，**将 Bot 提升为管理员**（赋予发消息和删除消息权限）。
...
如果你不希望群聊或私聊被大量的敏感词拦截/防刷日志刷屏：
1. 额外新建一个“拦截日志频道/群组”（获取其 ID，例如 `-100999999999`）。
2. 在环境变量中配置：
   - `ENV_FORWARD_CHAT_ID` = `你的客服群ID`
...
- **设置备注**：回复客人留言发送 `/tag VIP买家`，或发送 `/tag 用户ID VIP买家` 为客人添加标签。后续该客人留言时，通知卡片会自动显示备注名（`客人: 张三 [VIP买家] (123456)`）。输入 `/tag clear` 可清除备注。
